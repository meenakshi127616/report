	...
	<body class='extent ${theme} hide-overflow'>
		...
		<nav>
			<div class='logo-container'>
				<a class='logo-content' href='http://www.lochbridge.com'>
					<span>Lochbridge</span>
				</a>
									...
									<#list report.testList as extentTest>
										...
										<li class='collection-item test displayed active ${test.status} <#if test.nodeList?size != 0>hasChildren</#if>' extentid='${test.id?string}'>
											<div class='test-head'>
												...
												<#if test.nodeList?size == 0><span class='test-status label right outline capitalize ${test.status}'>${test.status}</span></#if>
																	...
																	<li class='displayed ${node.status} node-${depth}x'>
																		<div class='collapsible-header test-node ${node.status}'>
																			<div class='right test-info'>
																				...
																				<#if depth == 2><span class='test-status label outline capitalize ${node.status}'>${node.status}</span></#if>
																			</div>
	...
	<script>
		<#if report.configurationMap??>
			...
			/*function refreshCustomData() {
				...
				//totalTests = $('.test.displayed .node-list > li.displayed.node-2x, .test-node').length;
				...
				console.log("fatalTests " + fatalTests);
			...
			*/
			...
			//setTimeout(function(){ refreshCustomData();}, 200);
